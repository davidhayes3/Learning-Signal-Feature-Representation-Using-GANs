...
def save_imgs(gen_imgs, epoch):
    ...
    for i in range(r):
        for j in range(c):
            ...
            axs[i, j].axis('off')
            count += 1
    fig.savefig("mnist_dc_bigan_%d.png" % epoch)
    plt.close()
...
for class_index in range(num_classes):
    ...
    index = indices[0]
    test_digit_indices = np.concatenate(test_digit_indices, index)
...
for i in range(r):
    for j in range(c):
        ...
        axs[i, j].axis('off')
        count += 1
fig.savefig('mnist_bigan_recons_%d_epochs_%d_bs.png' % (epochs, batch_size))
plt.close()
...
plt.gray()
plt.savefig('mnist_bigan_recons2_%d_epochs_%d_bs.png' % (epochs, batch_size))
plt.show()
